(ns tenkiwi.gamemasters.hijinx
  "This is the game logic for Hints and Hijinx investigation games"
  (:require
...
(defn start-game [room-id {:keys [game-url concept]
  ...
  (let [decks      (util/gather-decks game-url)
        ...
        initial-state (-> {:game-type :hijinx
                           :epsiode   episode}
                          ...
                          (clock-list/initial-state {:allow-new? false
                                                     :clocks     [{:title    (get-in episode [:options :doom-track-name] "Doom Track")
                                                                   ...
                                                                   :min      1
                                                                   :current  2}]})
                          (dice-bag/initial-state {:shortcuts   [{:formula "1d2"
                                                                  :text    "24"}
                                                                 {:formula "1d4"
                                                                  :text    "d4"}
                                                                 {:formula "1d6"
