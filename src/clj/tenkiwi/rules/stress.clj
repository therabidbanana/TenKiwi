...
(defn initial-state [starting-state
                     {:keys [players score-min score-max default]
                      :or   {score-min 0 score-max 4 default 0}
                      :as   options}]
  (let [scores (reduce
                (fn [a b]
...
(defn upstress! [{{:keys [scores score-max]} $
                  :as              game}
                 path]
...
(defn downstress! [{{:keys [scores score-min]} $
                    :as              game}
                   path]
  (let [current-score (get-in scores path)
...
(defn render-display [state]
  (-> state
      (update :display assoc :player-stress (->scores state))))
...
(defn- build-score-row [name id score]
  {:id id
   ...
   :score score
   :actions [{:text " - "
              :action :downstress-player
              :params {:player-id id}}
             {:text " + "
              :action :upstress-player
              :params {:player-id id}}]})
...
(defn render-scoreboard-display [state]
  (let [names      (character-sheets/->player-names state)
        ...
        scoreboard (mapv build-score-row
                         ...
                         (vals scores))]
    (-> state
        (update :display assoc :player-scoreboard scoreboard))))
